...
//#endregion
//#region node_modules/@theatre/dataverse/dist/index.js
var require_dist = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var reEscapeChar = /\\(\\)?/g;
	var stringToPath_default = memoizeCapped_default(function(string) {
		var result = [];
	...
	var getPrototype_default = overArg(Object.getPrototypeOf, Object);
	...
	function isPlainObject(value) {
		if (!isObjectLike_default(value) || baseGetTag_default(value) != objectTag) return false;
	...
	var proxyHandler = { get(pointerKey, prop) {
	...
	var hoop = (s, path, reducer) => {
		if (path.length === 0) return reducer(s);
		if (Array.isArray(s)) {
			...
			const newVal = hoop(oldVal, restOfPath, reducer);
			if (oldVal === newVal) return s;
			return __spreadProps(__spreadValues({}, s), { [key]: newVal });
		} else {
			const [key, ...restOfPath] = path;
	...
	function createMechanism() {
		...
		const popCollector2 = (collector) => {
			if (stack.peek() !== collector) throw new Error(`Popped collector is not on top of the stack`);
			stack.pop();
		};
		...
		const stopIgnoringDependencies2 = () => {
			if (stack.peek() !== noopCollector) console.warn("This should never happen");
			else stack.pop();
		};
		const reportResolutionStart2 = (d) => {
			const possibleCollector = stack.peek();
			if (possibleCollector) possibleCollector(d);
			stack.push(noopCollector);
		};
	...
	function getSharedMechanism() {
		...
		if (root2) {
			...
			if (existingMechanism && typeof existingMechanism === "object" && existingMechanism.type === "Dataverse_discoveryMechanism") return existingMechanism;
			else {
				const mechanism = createMechanism();
				root2[varName] = mechanism;
				return mechanism;
			}
		} else return createMechanism();
	}
	var { startIgnoringDependencies, stopIgnoringDependencies, reportResolutionEnd, reportResolutionStart, pushCollector, popCollector } = getSharedMechanism();
	var voidFn = () => {};
	var HotHandle = class {
		constructor(_fn, _prismInstance) {
			...
			for (const d of this._dependencies) d._addDependent(this._reactToDependencyGoingStale);
		...
		destroy() {
			for (const d of this._dependencies) d._removeDependent(this._reactToDependencyGoingStale);
			cleanupScopeStack(this._scope);
		}
		getValue() {
			if (!this._isFresh) {
				this._lastValue = this._recalculate();
				this._isFresh = true;
				this._didMarkDependentsAsStale = false;
				this._forciblySetToStale = false;
			}
		...
		_recalculate() {
			...
			if (!this._forciblySetToStale) {
				if (this._possiblyStaleDeps.size > 0) {
					...
					startIgnoringDependencies();
					for (const dep of this._possiblyStaleDeps) if (this._cacheOfDendencyValues.get(dep) !== dep.getValue()) {
						anActuallyStaleDepWasFound = true;
						break;
					}
					stopIgnoringDependencies();
					this._possiblyStaleDeps.clear();
					if (!anActuallyStaleDepWasFound) return this._lastValue;
				}
			}
			...
			try {
				...
				if (hookScopeStack.pop() !== this._scope) console.warn(`The Prism hook stack has slipped. This is a bug.`);
			}
			popCollector(collector);
			for (const dep of this._dependencies) if (!newDeps.has(dep)) this._removeDependency(dep);
			this._dependencies = newDeps;
			startIgnoringDependencies();
			for (const dep of newDeps) this._cacheOfDendencyValues.set(dep, dep.getValue());
			stopIgnoringDependencies();
			return value;
		}
		forceStale() {
		...
		_markAsStale() {
			...
			this._isFresh = false;
			for (const dependent of this._dependents) dependent(this._prismInstance);
		}
		_addDependency(d) {
	...
	var PrismInstance = class {
		...
		_addDependent(d) {
			if (!this._state.hot) this._goHot();
			this._state.handle.addDependent(d);
		}
		_goHot() {
			this._state = {
				hot: true,
				handle: new HotHandle(this._fn, this)
			};
		}
		_removeDependent(d) {
			const state2 = this._state;
			if (!state2.hot) return;
		...
		getValue() {
			...
			const state2 = this._state;
			let val2;
			if (state2.hot) val2 = state2.handle.getValue();
			else val2 = calculateColdPrism(this._fn);
	...
	var HotScope = class {
		...
		ref(key, initialValue) {
			...
			if (ref2 !== void 0) return ref2;
				...
				const ref3 = { current: initialValue };
		...
		sub(key) {
			if (!this.subs[key]) this.subs[key] = new HotScope(this._hotHandle);
			return this.subs[key];
		}
		cleanupEffects() {
			for (const effect2 of this.effects.values()) safelyRun(effect2.cleanup, void 0);
			this.effects.clear();
		}
		source(subscribe, getValue2) {
			this.effect("$$source/blah", () => {
				return subscribe(() => {
					this._hotHandle.forceStale();
				});
			}, [subscribe]);
	...
	function cleanupScopeStack(scope2) {
		for (const sub2 of Object.values(scope2.subs)) cleanupScopeStack(sub2);
	...
	function ref(key, initialValue) {
		...
		if (!scope2) throw new Error(`prism.ref() is called outside of a prism() call.`);
		return scope2.ref(key, initialValue);
	}
	function effect(key, cb, deps) {
		const scope2 = hookScopeStack.peek();
		if (!scope2) throw new Error(`prism.effect() is called outside of a prism() call.`);
		return scope2.effect(key, cb, deps);
	}
	function depsHaveChanged(oldDeps, newDeps) {
		if (oldDeps === void 0 || newDeps === void 0) return true;
		const len = oldDeps.length;
		if (len !== newDeps.length) return true;
		for (let i = 0; i < len; i++) if (oldDeps[i] !== newDeps[i]) return true;
		return false;
	}
	function memo(key, fn, deps) {
		const scope2 = hookScopeStack.peek();
		if (!scope2) throw new Error(`prism.memo() is called outside of a prism() call.`);
		return scope2.memo(key, fn, deps);
	}
	function state(key, initialValue) {
		const scope2 = hookScopeStack.peek();
		if (!scope2) throw new Error(`prism.state() is called outside of a prism() call.`);
		return scope2.state(key, initialValue);
	}
	function ensurePrism() {
		if (!hookScopeStack.peek()) throw new Error(`The parent function is called outside of a prism() call.`);
	}
	function scope(key, fn) {
		const parentScope = hookScopeStack.peek();
		if (!parentScope) throw new Error(`prism.scope() is called outside of a prism() call.`);
	...
	function source(subscribe, getValue2) {
		const scope2 = hookScopeStack.peek();
		if (!scope2) throw new Error(`prism.source() is called outside of a prism() call.`);
	...
	function calculateColdPrism(fn) {
		...
		try {
			...
			if (hookScopeStack.pop() !== scope2) console.warn(`The Prism hook stack has slipped. This is a bug.`);
		}
		return value;
	}
	prism.ref = ref;
	...
	var getKeyOfValue = (v, key, vType = getTypeOfValue(v)) => {
		if (vType === 0 && typeof key === "string") return v[key];
		else if (vType === 1 && isValidArrayIndex(key)) return v[key];
		else return;
	};
	...
	var Scope = class {
		...
		_checkForGC() {
			...
			if (this._parent) this._parent.removeChild(last_default(this._path));
		}
	};
	var Atom = class {
		...
		getByPointer(pointerOrFn) {
			const path = getPointerParts(isPointer(pointerOrFn) ? pointerOrFn : pointerOrFn(this.pointer)).path;
		...
		_checkUpdates(scope2, oldState, newState) {
			...
			for (const cb of scope2.identityChangeListeners) cb(newState);
		...
		_getOrCreateScopeForPath(path) {
			...
			for (const pathEl of path) curScope = curScope.getOrCreateChild(pathEl);
			return curScope;
		}
	...
	var pointerToPrism = (pointer2) => {
		...
		if (!prismInstance) {
			...
			if (!isPointerToPrismProvider(root2)) throw new Error(`Cannot run pointerToPrism() on a pointer whose root is not an PointerToPrismProvider`);
	...
	var val = (input) => {
		if (isPointer(input)) return pointerToPrism(input).getValue();
		else if (isPrism(input)) return input.getValue();
		else return input;
	};
	function* iterateAndCountTicks(pointerOrPrism) {
		let d;
		if (isPointer(pointerOrPrism)) d = pointerToPrism(pointerOrPrism);
		else if (isPrism(pointerOrPrism)) d = pointerOrPrism;
		else throw new Error(`Only pointers and prisms are supported`);
	...
	var EMPTY_TICKS_BEFORE_GOING_DORMANT = 180;
	var Ticker = class {
		constructor(_conf) {
			this._conf = _conf;
			this._ticking = false;
		...
		onThisOrNextTick(fn) {
			...
			if (this._dormant) this._goActive();
		}
		onNextTick(fn) {
		...
		get time() {
			if (this._ticking) return this._timeAtCurrentTick;
		...
		_goActive() {
			...
			this._dormant = false;
			(_b = (_a = this._conf) == null ? void 0 : _a.onActive) == null || _b.call(_a);
		}
		_goDormant() {
			var _a, _b;
			if (this._dormant) return;
			this._dormant = true;
			this._numberOfDormantTicks = 0;
			(_b = (_a = this._conf) == null ? void 0 : _a.onDormant) == null || _b.call(_a);
		}
		tick(t = performance.now()) {
			if (!(this instanceof Ticker)) throw new Error("ticker.tick must be called while bound to the ticker. As in, \"ticker.tick(time)\" or \"requestAnimationFrame((t) => ticker.tick(t))\" for performance.");
			this.__ticks++;
			if (!this._dormant) {
			...
			for (const v of this._scheduledForNextTick) this._scheduledForThisOrNextTick.add(v);
		...
		_tick(iterationNumber) {
			...
			if (iterationNumber > 10) console.warn("_tick() recursing for 10 times");
			if (iterationNumber > 100) throw new Error(`Maximum recursion limit for _tick()`);
			const oldSet = this._scheduledForThisOrNextTick;
			this._scheduledForThisOrNextTick = /* @__PURE__ */ new Set();
			for (const fn of oldSet) fn(time);
			if (this._scheduledForThisOrNextTick.size > 0) return this._tick(iterationNumber + 1);
		}
	};
	...
	var PointerProxy = class {
		...
		pointerToPrism(pointer2) {
			...
			return prism_default(() => {
				...
				return val(path.reduce((pointerSoFar, pathItem) => pointerSoFar[pathItem], currentPointer));
			});
		}
	};
}));
//#endregion
export { __commonJSMin as n, __require as r, require_dist as t };

//# sourceMappingURL=dist-Ch3_Wy7X.js.map