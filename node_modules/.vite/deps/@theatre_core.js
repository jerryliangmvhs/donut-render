import { n as __commonJSMin, t as require_dist$1 } from "./dist-Ch3_Wy7X.js";
//#region node_modules/@theatre/core/dist/index.js
var require_dist = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var require_UnitBezier = __commonJS({ "../node_modules/timing-function/lib/UnitBezier.js"(exports2, module2) {
		module2.exports = function() {
			...
			UnitBezier3.prototype._solveCurveX = function(x2, epsilon) {
				var d2, i2, t0 = void 0, t1 = void 0, t2 = void 0, x22 = void 0;
				d2 = void 0;
				i2 = void 0;
				t2 = x2;
				i2 = 0;
				while (i2 < 8) {
					...
					if (Math.abs(x22) < epsilon) return t2;
				...
				while (t0 < t1) {
					...
					if (Math.abs(x22 - x2) < epsilon) return t2;
					if (x2 > x22) t0 = t2;
					else t1 = t2;
					t2 = (t1 - t0) * .5 + t0;
				}
	...
	var require_levenshtein_edit_distance = __commonJS({ "../node_modules/levenshtein-edit-distance/index.js"(exports2, module2) {
		var cache2;
		var codes;
		cache2 = [];
		codes = [];
	...
	var require_propose = __commonJS({ "../node_modules/propose/propose.js"(exports2, module2) {
		...
		function propose2() {
			...
			for (var i2 = 0; i2 < len; ++i2) {
				if (ignoreCase) distance = levenshtein(word, dictionary[i2], true);
				else distance = levenshtein(word, dictionary[i2]);
				if (distance > word.length) ratio = 1 - distance / dictionary[i2].length;
				else ratio = 1 - distance / word.length;
				if (ratio > max_ratio) {
					max_ratio = ratio;
	...
	var require_fast_deep_equal = __commonJS({ "../node_modules/fast-deep-equal/index.js"(exports2, module2) {
		"use strict";
	...
	var import_dataverse = require_dist$1();
	...
	var projectsSingleton_default = new ProjectsSingleton();
	...
	function memoizeCapped(func) {
		var result = memoize_default(func, function(key) {
			if (cache2.size === MAX_MEMOIZE_SIZE) cache2.clear();
			return key;
		});
	...
	var reEscapeChar = /\\(\\)?/g;
	var stringToPath_default = memoizeCapped_default(function(string2) {
		var result = [];
		if (string2.charCodeAt(0) === 46) result.push("");
	...
	var SimpleCache = class {
		...
		*/
		get(key, producer) {
			if (this.has(key)) return this._values[key];
			else {
	...
	var import_dataverse6 = require_dist$1();
	var defineProperty_default = function() {
		try {
	...
	function baseAssignValue(object, key, value) {
		if (key == "__proto__" && defineProperty_default) defineProperty_default(object, key, {
			"configurable": true,
			"enumerable": true,
			"value": value,
			"writable": true
	...
	function assignValue(object, key, value) {
		...
		if (!(hasOwnProperty5.call(object, key) && eq_default(objValue, value)) || value === void 0 && !(key in object)) baseAssignValue_default(object, key, value);
	}
	...
	function baseSet(object, path, value, customizer) {
		...
		while (nested != null && ++index < length) {
			...
			if (index != lastIndex) {
				...
				if (newValue === void 0) newValue = isObject_default(objValue) ? objValue : isIndex_default(path[index + 1]) ? [] : {};
			}
	...
	function getDefaultsOfPropTypeConfig(config) {
		if (cachedDefaults.has(config)) return cachedDefaults.get(config);
	...
	function generateDefaultsForEnum(config) {
		const defaults = { $case: config.defaultCase };
		for (const [case_, caseConf] of Object.entries(config.cases)) defaults[case_] = getDefaultsOfPropTypeConfig(caseConf);
		return defaults;
	}
	function generateDefaultsForCompound(config) {
		const defaults = {};
		for (const [key, propConf] of Object.entries(config.props)) defaults[key] = getDefaultsOfPropTypeConfig(propConf);
		return defaults;
	}
	var import_dataverse2 = require_dist$1();
	...
	function interpolationTripleAtPosition(ctx, trackP, timeD) {
		return (0, import_dataverse2.prism)(() => {
			...
			return import_dataverse2.prism.memo("driver", () => {
				if (!track) return (0, import_dataverse2.prism)(() => void 0);
				else if (track.type === "BasicKeyframedTrack") return _forKeyframedTrack(ctx, track, timeD);
				else {
	...
	function _forKeyframedTrack(ctx, track, timeD) {
		return (0, import_dataverse2.prism)(() => {
			...
			if (!state.started || time < state.validFrom || state.validTo <= time) stateRef.current = state = updateState(ctx, timeD, track);
	...
	function updateState(ctx, progressionD, track) {
		...
		while (true) {
			...
			if (progression < currentKeyframe.position) if (currentKeyframeIndex === 0) return states.beforeFirstKeyframe(currentKeyframe);
			else {
				ctx.logger.error("Bug here");
				return states.error;
			}
			else if (currentKeyframe.position === progression) if (isLastKeyframe) return states.lastKeyframe(currentKeyframe);
			else return states.between(currentKeyframe, track.keyframes[currentKeyframeIndex + 1], progressionD);
			else if (currentKeyframeIndex === track.keyframes.length - 1) return states.lastKeyframe(currentKeyframe);
			else {
	...
	function deepMergeWithCache(base, override, cache2) {
		const possibleCachedValue = cache2.get(base);
		if (possibleCachedValue && possibleCachedValue.override === override) return possibleCachedValue.merged;
	...
	function pointerDeep(base, toAppend) {
		let p2 = base;
		for (const k2 of toAppend) p2 = p2[k2];
		return p2;
	}
	var import_dataverse3 = require_dist$1();
	var valToAtom = (key, vals) => {
		const a2 = import_dataverse3.prism.memo(key, () => new import_dataverse3.Atom(vals), []);
		a2.set(vals);
		return a2;
	};
	var import_dataverse5 = require_dist$1();
	var import_dataverse4 = require_dist$1();
	var reWhitespace = /\s/;
	...
	var INFINITY3 = Infinity;
	var MAX_INTEGER = 17976931348623157e292;
	function toFinite(value) {
		if (!value) return value === 0 ? value : 0;
		value = toNumber_default(value);
		if (value === INFINITY3 || value === -INFINITY3) return (value < 0 ? -1 : 1) * MAX_INTEGER;
	...
	var WeakMap_default = getNative_default(root_default, "WeakMap");
	var objectCreate = Object.create;
	var baseCreate_default = function() {
		function object() {}
	...
	function copyObject(source, props, object, customizer) {
		...
		while (++index < length) {
			...
			if (isNew) baseAssignValue_default(object, key, newValue);
			else assignValue_default(object, key, newValue);
		}
		return object;
	}
	...
	function isPrototype(value) {
		...
		return value === (typeof Ctor == "function" && Ctor.prototype || objectProto7);
	}
	var isPrototype_default = isPrototype;
	function baseTimes(n2, iteratee) {
		var index = -1, result = Array(n2);
		while (++index < n2) result[index] = iteratee(index);
		return result;
	}
	...
	var isArguments_default = baseIsArguments_default(function() {
		return arguments;
	...
	var Buffer2 = freeModule && freeModule.exports === freeExports ? root_default.Buffer : void 0;
	var isBuffer_default = (Buffer2 ? Buffer2.isBuffer : void 0) || stubFalse_default;
	...
	var freeProcess = freeModule2 && freeModule2.exports === freeExports2 && freeGlobal_default.process;
	var nodeUtil_default = function() {
		try {
	...
	var isTypedArray_default = nodeIsTypedArray ? baseUnary_default(nodeIsTypedArray) : baseIsTypedArray_default;
	var hasOwnProperty7 = Object.prototype.hasOwnProperty;
	function arrayLikeKeys(value, inherited) {
		var isArr = isArray_default(value), isArg = !isArr && isArguments_default(value), isBuff = !isArr && !isArg && isBuffer_default(value), isType = !isArr && !isArg && !isBuff && isTypedArray_default(value), skipIndexes = isArr || isArg || isBuff || isType, result = skipIndexes ? baseTimes_default(value.length, String) : [], length = result.length;
		for (var key in value) if ((inherited || hasOwnProperty7.call(value, key)) && !(skipIndexes && (key == "length" || isBuff && (key == "offset" || key == "parent") || isType && (key == "buffer" || key == "byteLength" || key == "byteOffset") || isIndex_default(key, length)))) result.push(key);
		return result;
	}
	...
	var nativeKeys_default = overArg_default(Object.keys, Object);
	var hasOwnProperty8 = Object.prototype.hasOwnProperty;
	function baseKeys(object) {
		if (!isPrototype_default(object)) return nativeKeys_default(object);
		var result = [];
		for (var key in Object(object)) if (hasOwnProperty8.call(object, key) && key != "constructor") result.push(key);
		return result;
	}
	...
	function baseKeysIn(object) {
		if (!isObject_default(object)) return nativeKeysIn_default(object);
		var isProto = isPrototype_default(object), result = [];
		for (var key in object) if (!(key == "constructor" && (isProto || !hasOwnProperty9.call(object, key)))) result.push(key);
		return result;
	}
	...
	var getPrototype_default = overArg_default(Object.getPrototypeOf, Object);
	...
	function isPlainObject(value) {
		if (!isObjectLike_default(value) || baseGetTag_default(value) != objectTag2) return false;
	...
	function baseClamp(number2, lower, upper) {
		if (number2 === number2) {
			if (upper !== void 0) number2 = number2 <= upper ? number2 : upper;
			if (lower !== void 0) number2 = number2 >= lower ? number2 : lower;
		}
		return number2;
	}
	...
	function Stack(entries) {
		this.size = (this.__data__ = new ListCache_default(entries)).size;
	}
	...
	var Buffer3 = freeModule3 && freeModule3.exports === freeExports3 ? root_default.Buffer : void 0;
	...
	var getSymbols_default = !nativeGetSymbols ? stubArray_default : function(object) {
	...
	var getSymbolsIn_default = !Object.getOwnPropertySymbols ? stubArray_default : function(object) {
		var result = [];
		while (object) {
	...
	var DataView_default = getNative_default(root_default, "DataView");
	var Promise_default = getNative_default(root_default, "Promise");
	var Set_default = getNative_default(root_default, "Set");
	...
	if (DataView_default && getTag(new DataView_default(/* @__PURE__ */ new ArrayBuffer(1))) != dataViewTag2 || Map_default && getTag(new Map_default()) != mapTag2 || Promise_default && getTag(Promise_default.resolve()) != promiseTag || Set_default && getTag(new Set_default()) != setTag2 || WeakMap_default && getTag(new WeakMap_default()) != weakMapTag2) getTag = function(value) {
	...
	var Uint8Array_default = root_default.Uint8Array;
	...
	function initCloneByTag(object, tag, isDeep) {
		...
		switch (tag) {
			case arrayBufferTag2: return cloneArrayBuffer_default(object);
			case boolTag2:
			case dateTag2: return new Ctor(+object);
			case dataViewTag3: return cloneDataView_default(object, isDeep);
			case float32Tag2:
			case float64Tag2:
			case int8Tag2:
			case int16Tag2:
			...
			case uint16Tag2:
			case uint32Tag2: return cloneTypedArray_default(object, isDeep);
			case mapTag3: return new Ctor();
			case numberTag2:
			case stringTag2: return new Ctor(object);
			case regexpTag2: return cloneRegExp_default(object);
			case setTag3: return new Ctor();
			case symbolTag2: return cloneSymbol_default(object);
		}
	}
	...
	var isMap_default = nodeIsMap ? baseUnary_default(nodeIsMap) : baseIsMap_default;
	...
	var isSet_default = nodeIsSet ? baseUnary_default(nodeIsSet) : baseIsSet_default;
	...
	function baseClone(value, bitmask, customizer, key, object, stack) {
		...
		if (isArr) {
			...
			if (!isDeep) return copyArray_default(value, result);
		} else {
			var tag = getTag_default(value), isFunc = tag == funcTag3 || tag == genTag2;
			if (isBuffer_default(value)) return cloneBuffer_default(value, isDeep);
		...
		var props = isArr ? void 0 : (isFull ? isFlat ? getAllKeysIn_default : getAllKeys_default : isFlat ? keysIn_default : keys_default)(value);
	...
	function equalByTag(object, other, tag, bitmask, customizer, equalFunc, stack) {
		switch (tag) {
			...
			case arrayBufferTag4:
				if (object.byteLength != other.byteLength || !equalFunc(new Uint8Array_default(object), new Uint8Array_default(other))) return false;
				return true;
			case boolTag4:
			case dateTag4:
			case numberTag4: return eq_default(+object, +other);
			case errorTag3: return object.name == other.name && object.message == other.message;
			case regexpTag4:
			case stringTag4: return object == other + "";
			case mapTag6: var convert = mapToArray_default;
			case setTag6:
				...
				stack["delete"](object);
				return result;
			case symbolTag4: if (symbolValueOf2) return symbolValueOf2.call(object) == symbolValueOf2.call(other);
		}
		return false;
	}
	...
	function equalObjects(object, other, bitmask, customizer, equalFunc, stack) {
		var isPartial = bitmask & COMPARE_PARTIAL_FLAG3, objProps = getAllKeys_default(object), objLength = objProps.length;
		if (objLength != getAllKeys_default(other).length && !isPartial) return false;
		var index = objLength;
		while (index--) {
			var key = objProps[index];
			if (!(isPartial ? key in other : hasOwnProperty12.call(other, key))) return false;
		}
	...
	function baseIteratee(value) {
		...
		if (typeof value == "object") return isArray_default(value) ? baseMatchesProperty_default(value[0], value[1]) : baseMatches_default(value);
	...
	function debounce(func, wait, options) {
		...
		if (typeof func != "function") throw new TypeError(FUNC_ERROR_TEXT2);
	...
	function baseRepeat(string2, n2) {
		var result = "";
		if (!string2 || n2 < 1 || n2 > MAX_SAFE_INTEGER3) return result;
		do {
			if (n2 % 2) result += string2;
			n2 = nativeFloor(n2 / 2);
			if (n2) string2 += string2;
		} while (n2);
		return result;
	}
	var baseRepeat_default = baseRepeat;
	var asciiSize_default = baseProperty_default("length");
	var rsAstralRange3 = "\\ud800-\\udfff";
	var rsComboRange3 = "\\u0300-\\u036f\\ufe20-\\ufe2f\\u20d0-\\u20ff";
	var rsVarRange3 = "\\ufe0e\\ufe0f";
	...
	function unicodeSize(string2) {
		...
		while (reUnicode2.test(string2)) ++result;
		return result;
	}
	...
	function createPadding(length, chars) {
		...
		if (charsLength < 2) return charsLength ? baseRepeat_default(chars, length) : chars;
		...
		*/
		...
		get value() {
			...
			if (KEEP_HOT_FOR_MS != null) {
				...
				if (this._keepHotUntapDebounce) this._keepHotUntapDebounce();
			}
			return der.getValue();
		}
	...
	function memoizeFn(producer) {
		...
		return (k2) => {
			if (!cache2.has(k2)) cache2.set(k2, producer(k2));
	...
	function getPropConfigByPath(parentConf, path) {
		...
		return getPropConfigByPath(parentConf.type === "enum" ? parentConf.cases[key] : parentConf.props[key], rest);
	}
	...
	var compoundHasSimpleDescendants = memoizeFn((conf) => {
		...
		for (const key in conf.props) {
			...
			if (isPropConfigComposite(subConf)) {
				if (compoundHasSimpleDescendants(subConf)) return true;
			} else return true;
		}
		return false;
	});
	var SheetObject = class {
		constructor(sheet, template, nativeObject) {
			...
			this.address = __spreadProps(__spreadValues({}, template.address), { sheetInstanceId: sheet.address.sheetInstanceId });
		...
		getValues() {
			return this._cache.get("getValues()", () => (0, import_dataverse5.prism)(() => {
				let final = deepMergeWithCache(deepMergeWithCache((0, import_dataverse5.val)(this.template.getDefaultValues()), (0, import_dataverse5.val)(this._initialValue.pointer), import_dataverse5.prism.memo("withInitialCache", () => /* @__PURE__ */ new WeakMap(), [])), (0, import_dataverse5.val)(this.template.getStaticValues()), import_dataverse5.prism.memo("withStatics", () => /* @__PURE__ */ new WeakMap(), []));
				let sequenced;
				{
					...
					final = deepMergeWithCache(final, sequenced, withSeqsCache);
				}
	...
	var DEFAULTS = {
		...
		named(parent2, name, key) {
			return this.create({ names: [...parent2.names, {
				name,
				key
			}] });
		},
		style: {
			...
			collapsed(name) {
				...
				if (!this.cssMemo.has(collapsed)) this.cssMemo.set(collapsed, this.css(name));
				return collapsed;
			},
			css(name) {
				var _a, _b, _c, _d;
				...
				if ((_c = this.bold) == null ? void 0 : _c.test(name)) css += ";font-weight:600";
				if ((_d = this.italic) == null ? void 0 : _d.test(name)) css += ";font-style:italic";
	...
	function configNamedToKeyed(namedFn, source) {
		const names = [];
		for (let { name, key } of source.names) names.push(key == null ? name : "".concat(name, " (").concat(key, ")"));
	...
	function iterateOnCompound(path, config, map) {
		for (const [key, subConf] of Object.entries(config.props)) if (!isPropConfigComposite(subConf)) {
	...
	function iterateOnAny(path, config, map) {
		if (config.type === "compound") iterateOnCompound(path, config, map);
		else if (config.type === "enum") throw new Error("Enums aren't supported yet");
		else map.set(JSON.stringify(path), map.size);
	}
		...
		*/
		getDefaultValues() {
			return this._cache.get("getDefaultValues()", () => (0, import_dataverse6.prism)(() => {
				return getPropDefaultsOfSheetObject((0, import_dataverse6.val)(this.configPointer));
			}));
		}
		/**
		...
		*/
		getStaticValues() {
			return this._cache.get("getStaticValues", () => (0, import_dataverse6.prism)(() => {
				...
				return (0, import_dataverse6.val)(this.configPointer).deserializeAndSanitize(json) || {};
			}));
		}
		/**
		...
		getArrayOfValidSequenceTracks() {
			return this._cache.get("getArrayOfValidSequenceTracks", () => (0, import_dataverse6.prism)(() => {
				...
				arrayOfIds.sort((a2, b2) => {
					...
					if (mapping.get(JSON.stringify(pathToPropA)) > mapping.get(JSON.stringify(pathToPropB))) return 1;
					return -1;
				});
				if (arrayOfIds.length === 0) return emptyArray;
				else return arrayOfIds;
			}));
		}
		/**
		...
		*/
		getMapOfValidSequenceTracks_forStudio() {
			return this._cache.get("getMapOfValidSequenceTracks_forStudio", () => (0, import_dataverse6.prism)(() => {
				...
				let map = {};
				for (const { pathToProp, trackId } of arr) set_default(map, pathToProp, trackId);
				return map;
			}));
		}
		/**
			...
			return this._cache.get("getStaticButNotSequencedOverrides", () => (0, import_dataverse6.prism)(() => {
				...
				for (const { pathToProp } of arrayOfValidSequenceTracks) {
					...
					while (parentPath.length > 0) {
						if (!isObjectEmpty(getDeep(staticButNotSequencedOverrides, parentPath))) break;
						unset_default(staticButNotSequencedOverrides, parentPath);
						parentPath = parentPath.slice(0, -1);
					}
				}
				if (isObjectEmpty(staticButNotSequencedOverrides)) return;
				else return staticButNotSequencedOverrides;
			}));
		}
		getDefaultsAtPointer(pointer3) {
			const { path } = (0, import_dataverse6.getPointerParts)(pointer3);
			return getDeep(this.getDefaultValues().getValue(), path);
		}
	};
	...
	var import_dataverse15 = require_dist$1();
	...
	function didYouMean(str, dictionary, prepend = "Did you mean ", append = "?") {
		const p2 = (0, import_propose.default)(str, dictionary, { threshold: .7 });
		if (p2) return prepend + JSON.stringify(p2) + append;
		else return "";
	}
	var TheatreError = class extends Error {};
	var InvalidArgumentError = class extends TheatreError {};
	var import_dataverse10 = require_dist$1();
	var import_dataverse11 = require_dist$1();
	var import_dataverse12 = require_dist$1();
	var import_dataverse13 = require_dist$1();
	function defer() {
		let resolve;
		let reject;
	...
	var import_dataverse7 = require_dist$1();
	var DefaultPlaybackController = class {
		...
		play(iterationCount, range, rate, direction, ticker) {
			...
			const iterationLength = range[1] - range[0];
			{
				...
				if (startPos < range[0] || startPos > range[1]) {
					if (direction === "normal" || direction === "alternate") this._updatePositionInState(range[0]);
					else if (direction === "reverse" || direction === "alternateReverse") this._updatePositionInState(range[1]);
				} else if (direction === "normal" || direction === "alternate") {
					if (startPos === range[1]) this._updatePositionInState(range[0]);
				} else if (startPos === range[0]) this._updatePositionInState(range[1]);
			}
			...
			if (direction === "reverse" || direction === "alternateReverse") initialElapsedPos = range[1] - this.getCurrentPosition();
			const tick = (currentTickerTime) => {
				const elapsedTickerTimeInSeconds = Math.max(currentTickerTime - initialTickerTime, 0) / 1e3;
				...
				if (elapsedPos !== totalPlaybackLength) {
					...
					if (direction !== "normal") if (direction === "reverse") currentIterationPos = iterationLength - currentIterationPos;
					else {
						const isCurrentIterationNumberEven = iterationNumber % 2 === 0;
						if (direction === "alternate") {
							if (!isCurrentIterationNumberEven) currentIterationPos = iterationLength - currentIterationPos;
					...
					if (direction === "normal") this._updatePositionInState(range[1]);
					else if (direction === "reverse") this._updatePositionInState(range[0]);
					else {
						const isLastIterationEven = (iterationCount - 1) % 2 === 0;
						if (direction === "alternate") if (isLastIterationEven) this._updatePositionInState(range[1]);
						else this._updatePositionInState(range[0]);
						else if (isLastIterationEven) this._updatePositionInState(range[0]);
						else this._updatePositionInState(range[1]);
					}
					this.playing = false;
		...
		playDynamicRange(rangeD, ticker) {
			...
			const tick = (currentTickerTime) => {
				...
				if (lastPosition < range[0] || lastPosition > range[1]) this.gotoPosition(range[0]);
				else {
					let newPosition = lastPosition + elapsedSinceLastTickInSeconds;
					if (newPosition > range[1]) newPosition = range[0] + (newPosition - range[1]);
	...
	var import_dataverse8 = require_dist$1();
	...
	var AudioPlaybackController = class {
		...
		_loopInRange(range, ticker) {
			...
			const iterationLength = range[1] - range[0];
			if (startPos < range[0] || startPos > range[1]) this._updatePositionInState(range[0]);
			else if (startPos === range[1]) this._updatePositionInState(range[0]);
			...
			const tick = (currentTickerTime) => {
				let currentIterationPos = (Math.max(currentTickerTime - initialTickerTime, 0) / 1e3 * rate + initialElapsedPos) / iterationLength % 1 * iterationLength;
				this._updatePositionInState(currentIterationPos + range[0]);
				requestNextTick();
			};
			const requestNextTick = () => ticker.onNextTick(tick);
		...
		play(iterationCount, range, rate, direction, ticker) {
			...
			let startPos = this.getCurrentPosition();
			const iterationLength = range[1] - range[0];
			if (direction !== "normal") throw new InvalidArgumentError("Audio-controlled sequences can only be played in the \"normal\" direction. " + "'".concat(direction, "' given."));
			if (startPos < range[0] || startPos > range[1]) this._updatePositionInState(range[0]);
			else if (startPos === range[1]) this._updatePositionInState(range[0]);
			startPos = this.getCurrentPosition();
			const deferred = defer();
	...
	var import_dataverse9 = require_dist$1();
	var lastDriverId = 0;
	...
	function getCoreRafDriver() {
		if (!coreRafDriver) setCoreRafDriver(createBasicRafDriver());
	...
	function setCoreRafDriver(driver) {
		if (coreRafDriver) throw new Error("`setCoreRafDriver()` is already called.");
	...
	async function resolveAudioBuffer(args) {
		function getAudioContext() {
			...
			if (typeof window === "undefined") return Promise.resolve(ctx);
		...
		async function getAudioBuffer() {
			if (args.source instanceof AudioBuffer) return args.source;
			const decodedBufferDeferred = defer();
			if (typeof args.source !== "string") throw new Error("Error validating arguments to sequence.attachAudio(). args.source must either be a string or an instance of AudioBuffer.");
			let fetchResponse;
			try {
			...
			(await audioContextPromise).decodeAudioData(arrayBuffer, decodedBufferDeferred.resolve, decodedBufferDeferred.reject);
			let decodedBuffer2;
			try {
	...
	typeAsserter("Theatre_Project");
	typeAsserter("Theatre_Sheet");
	typeAsserter("Theatre_SheetTemplate");
	var isSheetObject = typeAsserter("Theatre_SheetObject");
	typeAsserter("Theatre_Sequence");
	typeAsserter("Theatre_SheetObjectTemplate");
	typeAsserter("Theatre_Project_PublicAPI");
	typeAsserter("Theatre_Sheet_PublicAPI");
	typeAsserter("Theatre_SheetObject_PublicAPI");
	...
	var Sequence = class {
		constructor(_project, _sheet, _lengthD, _subUnitsPerUnitD, playbackController) {
			...
			__publicField(this, "closestGridPosition", (posInUnitSpace) => {
				const gridLength = 1 / this.subUnitsPerUnit;
			...
			this._positionFormatterD = (0, import_dataverse13.prism)(() => {
				return new TimeBasedPositionFormatter((0, import_dataverse13.val)(this._subUnitsPerUnitD));
			});
		}
		get type() {
		...
		pointerToPrism(pointer3) {
			...
			if (path.length === 0) return (0, import_dataverse13.prism)(() => ({
			...
			if (path.length > 1) return (0, import_dataverse13.prism)(() => void 0);
			const [prop] = path;
			if (prop === "length") return this._lengthD;
			else if (prop === "subUnitsPerUnit") return this._subUnitsPerUnitD;
			else if (prop === "position") return this._positionD;
			else if (prop === "playing") return (0, import_dataverse13.prism)(() => {
				return (0, import_dataverse13.val)(this._prismOfStatePointer.getValue().playing);
			});
			else return (0, import_dataverse13.prism)(() => void 0);
		}
		/**
		...
		*/
		getKeyframesOfSimpleProp(prop) {
			...
			if (!isSheetObject(root2)) throw new InvalidArgumentError("Argument prop must be a pointer to a SheetObject property");
			...
			const id = trackIdByPropPath[encodePathToProp(path)];
			if (!id) return [];
		...
		*/
		...
		async play(conf, ticker) {
			...
			if (typeof range[0] !== "number" || range[0] < 0) throw new InvalidArgumentError("Argument conf.range[0] in sequence.play(conf) must be a positive number. ".concat(JSON.stringify(range[0]), " given."));
			if (range[0] >= sequenceDuration) throw new InvalidArgumentError("Argument conf.range[0] in sequence.play(conf) cannot be longer than the duration of the sequence, which is ".concat(sequenceDuration, "s. ").concat(JSON.stringify(range[0]), " given."));
			if (typeof range[1] !== "number" || range[1] <= 0) throw new InvalidArgumentError("Argument conf.range[1] in sequence.play(conf) must be a number larger than zero. ".concat(JSON.stringify(range[1]), " given."));
			...
			if (range[1] <= range[0]) throw new InvalidArgumentError("Argument conf.range[1] in sequence.play(conf) must be larger than conf.range[0]. ".concat(JSON.stringify(range), " given."));
			const iterationCount = conf && typeof conf.iterationCount === "number" ? conf.iterationCount : 1;
			if (!(Number.isInteger(iterationCount) && iterationCount > 0) && iterationCount !== Infinity) throw new InvalidArgumentError("Argument conf.iterationCount in sequence.play(conf) must be an integer larger than 0. ".concat(JSON.stringify(iterationCount), " given."));
			const rate = conf && typeof conf.rate !== "undefined" ? conf.rate : 1;
			if (typeof rate !== "number" || rate === 0) throw new InvalidArgumentError("Argument conf.rate in sequence.play(conf) must be a number larger than 0. ".concat(JSON.stringify(rate), " given."));
			if (rate < 0) throw new InvalidArgumentError("Argument conf.rate in sequence.play(conf) must be a number larger than 0. ".concat(JSON.stringify(rate), " given. If you want the animation to play backwards, try setting conf.direction to 'reverse' or 'alternateReverse'."));
			const direction = conf && conf.direction ? conf.direction : "normal";
			if (possibleDirections.indexOf(direction) === -1) throw new InvalidArgumentError("Argument conf.direction in sequence.play(conf) must be one of ".concat(JSON.stringify(possibleDirections), ". ").concat(JSON.stringify(direction), " given. ").concat(didYouMean(direction, possibleDirections)));
	...
	var TimeBasedPositionFormatter = class {
		...
		formatForPlayhead(posInUnitSpace) {
			...
			if (p2 >= frameLength) {
				...
				p2 = p2 % frameLength;
			} else if (p2 / frameLength > .98) {
				s2 += padStart_default(1 .toString(), 2, "0") + "f";
				p2 = p2 % frameLength;
			} else s2 += "00f";
	...
	var userReadableTypeOfValue = (v2) => {
		if (typeof v2 === "string") return "string(\"".concat(ellipsify(v2, 10), "\")");
		else if (typeof v2 === "number") return "number(".concat(ellipsify(String(v2), 10), ")");
		else if (v2 === null) return "null";
		else if (v2 === void 0) return "undefined";
		else if (typeof v2 === "boolean") return String(v2);
		else if (Array.isArray(v2)) return "array";
		else if (typeof v2 === "object") return "object";
		else return "unknown";
	};
	var userReadableTypeOfValue_default = userReadableTypeOfValue;
	function rgba2hex(rgba2, { removeAlphaIfOpaque = false } = {}) {
		const alpha = (rgba2.a * 255 | 256).toString(16).slice(1);
		const hex = (rgba2.r * 255 | 256).toString(16).slice(1) + (rgba2.g * 255 | 256).toString(16).slice(1) + (rgba2.b * 255 | 256).toString(16).slice(1) + (removeAlphaIfOpaque && alpha === "ff" ? "" : alpha);
		return "#".concat(hex);
	}
	function decorateRgba(rgba2) {
		return __spreadProps(__spreadValues({}, rgba2), { toString() {
	...
	function linearSrgbToSrgb(rgba2) {
		function compress(x2) {
			if (x2 >= .0031308) return 1.055 * x2 ** (1 / 2.4) - .055;
	...
	function srgbToLinearSrgb(rgba2) {
		function expand(x2) {
			if (x2 >= .04045) return ((x2 + .055) / 1.055) ** 2.4;
	...
	function linearSrgbToOklab(rgba2) {
		let l2 = .4122214708 * rgba2.r + .5363325363 * rgba2.g + .0514459929 * rgba2.b;
		let m = .2119034982 * rgba2.r + .6806995451 * rgba2.g + .1073969566 * rgba2.b;
		let s2 = .0883024619 * rgba2.r + .2817188376 * rgba2.g + .6299787005 * rgba2.b;
		let l_ = Math.cbrt(l2);
		let m_ = Math.cbrt(m);
		let s_ = Math.cbrt(s2);
		return {
	...
	function oklabToLinearSrgb(laba) {
		let l_ = laba.L + .3963377774 * laba.a + .2158037573 * laba.b;
		let m_ = laba.L - .1055613458 * laba.a - .0638541728 * laba.b;
		let s_ = laba.L - .0894841775 * laba.a - 1.291485548 * laba.b;
		let l2 = l_ * l_ * l_;
		let m = m_ * m_ * m_;
		let s2 = s_ * s_ * s_;
		return {
			r: 4.0767416621 * l2 - 3.3077115913 * m + .2309699292 * s2,
			g: -1.2684380046 * l2 + 2.6097574011 * m - .3413193965 * s2,
			b: -.0041960863 * l2 - .7034186147 * m + 1.707614701 * s2,
			a: laba.alpha
		};
	}
	...
	function toLonghandProp(p2) {
		if (typeof p2 === "number") return number(p2);
		else if (typeof p2 === "boolean") return boolean(p2);
		else if (typeof p2 === "string") return string(p2);
		else if (typeof p2 === "object" && !!p2) {
			if (isLonghandPropType(p2)) return p2;
			if (isPlainObject_default(p2)) return compound(p2);
			else throw new InvalidArgumentError("This value is not a valid prop type: ".concat(userReadableTypeOfValue_default(p2)));
		} else throw new InvalidArgumentError("This value is not a valid prop type: ".concat(userReadableTypeOfValue_default(p2)));
	}
	function sanitizeCompoundProps(props) {
		const sanitizedProps = {};
		if (typeof props !== "object" || !props) throw new InvalidArgumentError("t.compound() expects an object, like: {x: 10}. ".concat(userReadableTypeOfValue_default(props), " given."));
		for (const key of Object.keys(props)) {
			if (typeof key !== "string") throw new InvalidArgumentError("t.compound()'s keys must be all strings. ".concat(userReadableTypeOfValue_default(key), " given."));
			else if (key.length === 0 || !key.match(/^\w+$/)) throw new InvalidArgumentError("compound key ".concat(userReadableTypeOfValue_default(key), " is invalid. The keys must be alphanumeric and start with a letter."));
			else if (key.length > 64) throw new InvalidArgumentError("compound key ".concat(userReadableTypeOfValue_default(key), " is too long."));
			const val8 = props[key];
			if (isLonghandPropType(val8)) sanitizedProps[key] = val8;
			else sanitizedProps[key] = toLonghandProp(val8);
		}
		return sanitizedProps;
	}
	var validateCommonOpts = (fnCallSignature, opts) => {
		if (opts === void 0) return;
		if (typeof opts !== "object" || opts === null) throw new Error("opts in ".concat(fnCallSignature, " must either be undefined or an object."));
		if (Object.prototype.hasOwnProperty.call(opts, "label")) {
			const { label } = opts;
			if (typeof label !== "string") throw new Error("opts.label in ".concat(fnCallSignature, " should be a string. ").concat(userReadableTypeOfValue_default(label), " given."));
			if (label.trim().length !== label.length) throw new Error("opts.label in ".concat(fnCallSignature, " should not start/end with whitespace. \"").concat(label, "\" given."));
			if (label.length === 0) throw new Error("opts.label in ".concat(fnCallSignature, " should not be an empty string. If you wish to have no label, remove opts.label from opts."));
		}
	};
	var compound = (props, opts = {}) => {
		...
		return {
			...
			deserializeAndSanitize: (json) => {
				...
				if (deserializationCache.has(json)) return deserializationCache.get(json);
				const deserialized = {};
				let atLeastOnePropWasDeserialized = false;
				for (const [key, propConfig] of Object.entries(sanitizedProps)) if (Object.prototype.hasOwnProperty.call(json, key)) {
				...
				if (atLeastOnePropWasDeserialized) return deserialized;
			}
		};
	};
	...
	var _ensureFile = (val8) => {
		...
		let valid = true;
		if (typeof val8.id !== "string" && ![null, void 0].includes(val8.id)) valid = false;
	...
	var number = (defaultValue, opts = {}) => {
		...
		if (typeof defaultValue !== "number" || !isFinite(defaultValue)) throw new Error("Argument defaultValue in t.number(defaultValue) must be a number. ".concat(userReadableTypeOfValue_default(defaultValue), " given."));
		if (typeof opts === "object" && opts !== null) {
			if (Object.prototype.hasOwnProperty.call(opts, "range")) {
				if (!Array.isArray(opts.range)) throw new Error("opts.range in t.number(defaultValue, opts) must be a tuple of two numbers. ".concat(userReadableTypeOfValue_default(opts.range), " given."));
				if (opts.range.length !== 2) throw new Error("opts.range in t.number(defaultValue, opts) must have two elements. ".concat(opts.range.length, " given."));
				if (!opts.range.every((n2) => typeof n2 === "number" && !isNaN(n2))) throw new Error("opts.range in t.number(defaultValue, opts) must be a tuple of two numbers.");
				if (opts.range[0] >= opts.range[1]) throw new Error("opts.range[0] in t.number(defaultValue, opts) must be smaller than opts.range[1]. Given: ".concat(JSON.stringify(opts.range)));
			}
			if (Object.prototype.hasOwnProperty.call(opts, "nudgeMultiplier")) {
				if (typeof opts.nudgeMultiplier !== "number" || !isFinite(opts.nudgeMultiplier)) throw new Error("opts.nudgeMultiplier in t.number(defaultValue, opts) must be a finite number. ".concat(userReadableTypeOfValue_default(opts.nudgeMultiplier), " given."));
			}
			if (Object.prototype.hasOwnProperty.call(opts, "nudgeFn")) {
				if (typeof opts.nudgeFn !== "function") throw new Error("opts.nudgeFn in t.number(defaultValue, opts) must be a function. ".concat(userReadableTypeOfValue_default(opts.nudgeFn), " given."));
			}
		}
	...
	var rgba = (defaultValue = {
	...
	}, opts = {}) => {
		{
			...
			for (const p2 of [
				...
				"b",
				"a"
			]) if (!Object.prototype.hasOwnProperty.call(defaultValue, p2) || typeof defaultValue[p2] !== "number") valid = false;
			if (!valid) throw new Error("Argument defaultValue in t.rgba(defaultValue) must be of the shape { r: number; g: number, b: number, a: number; }.");
		}
		const sanitized = {};
	...
	var _sanitizeRgba = (val8) => {
		...
		for (const c2 of [
			...
			"b",
			"a"
		]) if (!Object.prototype.hasOwnProperty.call(val8, c2) || typeof val8[c2] !== "number") valid = false;
		if (!valid) return void 0;
		const sanitized = {};
		for (const c2 of [
			"r",
	...
	var _interpolateRgba = (left, right, progression) => {
		...
		return decorateRgba(linearSrgbToSrgb(oklabToLinearSrgb({
	...
	var boolean = (defaultValue, opts = {}) => {
		...
		if (typeof defaultValue !== "boolean") throw new Error("defaultValue in t.boolean(defaultValue) must be a boolean. ".concat(userReadableTypeOfValue_default(defaultValue), " given."));
		return {
			type: "boolean",
			default: defaultValue,
			valueType: null,
	...
	var string = (defaultValue, opts = {}) => {
		...
		if (typeof defaultValue !== "string") throw new Error("defaultValue in t.string(defaultValue) must be a string. ".concat(userReadableTypeOfValue_default(defaultValue), " given."));
		return {
			type: "string",
			default: defaultValue,
			valueType: null,
	...
	function stringLiteral(defaultValue, valuesAndLabels, opts = {}) {
		...
		return {
			...
			deserializeAndSanitize(json) {
				...
				if (Object.prototype.hasOwnProperty.call(valuesAndLabels, json)) return json;
				else return;
			}
		};
	}
	var defaultNumberNudgeFn = ({ config, deltaX, deltaFraction, magnitude }) => {
		var _a;
		const { range } = config;
		if (!config.nudgeMultiplier && range && !range.includes(Infinity) && !range.includes(-Infinity)) return deltaFraction * (range[1] - range[0]) * magnitude;
	...
	function validateAndSanitiseSlashedPathOrThrow(unsanitisedPath, fnName) {
		...
		if (validation) throw new InvalidArgumentError("The path in ".concat(fnName, "(").concat(typeof unsanitisedPath === "string" ? "\"".concat(unsanitisedPath, "\"") : "", ") is invalid because ").concat(validation));
		if (unsanitisedPath !== sanitisedPath) notify.warning("Invalid path provided to object", "The path in `".concat(fnName, "(\"").concat(unsanitisedPath, "\")` was sanitized to `\"").concat(sanitisedPath, "\"`.\n\n") + "Please replace the path with the sanitized one, otherwise it will likely break in the future.", [{
	...
	var TheatreSheet = class {
		...
		*/
			...
			if (existingObject) {
				{
					...
					if (prevConfig) {
						if (!(0, import_fast_deep_equal.default)(config, prevConfig)) if ((opts == null ? void 0 : opts.reconfigure) === true) {
						...
						} else throw new Error("You seem to have called sheet.object(\"".concat(key, "\", config) twice, with different values for `config`. ") + "This is disallowed because changing the config of an object on the fly would make it difficult to reason about.\n\n" + "You can fix this by either re-using the existing object, or calling sheet.object(\"".concat(key, "\", config) with the same config.\n\n") + "If you mean to reconfigure the object's config, set `{reconfigure: true}` in sheet.object(\"".concat(key, "\", config, {reconfigure: true})"));
					}
				}
				if (actions) existingObject.template._temp_setActions(actions);
				return existingObject.publicApi;
			} else {
				const sanitizedConfig = compound(config);
		...
		detachObject(key) {
			...
			if (!internal2.getObject(sanitizedPath)) {
	...
	var import_dataverse14 = require_dist$1();
	var Sheet = class {
		constructor(template, instanceId) {
			...
			this.address = __spreadProps(__spreadValues({}, template.address), { sheetInstanceId: this.instanceId });
		...
		*/
		createObject(objectKey, nativeObject, config, actions = {}) {
			const object = this.template.getObjectTemplate(objectKey, nativeObject, config, actions).createInstance(this, nativeObject, config);
		...
		getSequence() {
			if (!this._sequence) {
				const lengthD = (0, import_dataverse14.prism)(() => {
					return sanitizeSequenceLength((0, import_dataverse14.val)(this.project.pointers.historic.sheetsById[this.address.sheetId].sequence.length));
				});
				const subUnitsPerUnitD = (0, import_dataverse14.prism)(() => {
					return sanitizeSequenceSubUnitsPerUnit((0, import_dataverse14.val)(this.project.pointers.historic.sheetsById[this.address.sheetId].sequence.subUnitsPerUnit));
				});
	...
	var import_dataverse16 = require_dist$1();
	var import_dataverse17 = require_dist$1();
	...
	var globals_default = { currentProjectStateDefinitionVersion: "0.4.0" };
	async function initialiseProjectState(studio, project, onDiskState) {
		...
		studio.transaction(({ drafts }) => {
			...
			drafts.ahistoric.coreByProject[projectId] = { ahistoricStuff: "" };
			function useInitialState() {
				drafts.ephemeral.coreByProject[projectId].loadingState = { type: "loaded" };
			...
			if (!browserState) if (!onDiskState) useInitialState();
			else useOnDiskState(onDiskState);
			else if (!onDiskState) useBrowserState();
			else if (browserState.revisionHistory.indexOf(onDiskState.revisionHistory[0]) == -1) browserStateIsNotBasedOnDiskState(onDiskState);
			else useBrowserState();
		});
	}
	function noop2() {}
	function _coreLogger(config) {
		...
		if (config) {
			...
			else internal2.configureLogging({ dev: true });
		}
	...
	var Project = class {
		constructor(id, config = {}, publicApi) {
			...
			else if (typeof window === "undefined") {} else setTimeout(() => {
				if (!this._studio) throw new Error("Argument config.state in Theatre.getProject(\"".concat(id, "\", config) is empty. This is fine ") + "while you are using @theatre/core along with @theatre/studio. But since @theatre/studio " + "is not loaded, the state of project \"".concat(id, "\" will be empty.\n\n") + "To fix this, you need to add @theatre/studio into the bundle and export the project's state. Learn how to do that at https://www.theatrejs.com/docs/latest/manual/projects#state\n");
			}, 1e3);
		}
		attachToStudio(studio) {
			if (this._studio) if (this._studio !== studio) throw new Error("Project ".concat(this.address.projectId, " is already attached to studio ").concat(this._studio.address.studioId));
			else {
	...
	var _validateSym = (val8, thingy, range) => {
		if (typeof val8 !== "string") return "".concat(thingy, " must be a string. ").concat(userReadableTypeOfValue_default(val8), " given.");
		else if (val8.trim().length !== val8.length) return "".concat(thingy, " must not have leading or trailing spaces. '").concat(val8, "' given.");
		else if (val8.length < range[0] || val8.length > range[1]) return "".concat(thingy, " must have between ").concat(range[0], " and ").concat(range[1], " characters. '").concat(val8, "' given.");
	};
	var validateName = (name, thingy, shouldThrow = false) => {
		const result = _validateSym(name, thingy, [3, 32]);
		if (typeof result === "string" && shouldThrow) throw new InvalidArgumentError(result);
		else return result;
	};
	...
	var import_dataverse18 = require_dist$1();
	var import_dataverse19 = require_dist$1();
	function getProject(id, config = {}) {
		const existingProject = projectsSingleton_default.get(id);
		if (existingProject) {
			if (!(0, import_fast_deep_equal2.default)(config, existingProject.config)) throw new Error("You seem to have called Theatre.getProject(\"".concat(id, "\", config) twice, with different config objects. ") + "This is disallowed because changing the config of a project on the fly can lead to hard-to-debug issues.\n\nYou can fix this by either calling Theatre.getProject() once per projectId, or calling it multiple times but with the exact same config.");
			return existingProject.publicApi;
		}
		const plogger = _coreLogger().named("Project", id);
		...
		} else plogger._debug("no config.state");
		return new TheatreProject(id, config);
	}
	var shallowValidateOnDiskState = (projectId, s2) => {
		if (Array.isArray(s2) || s2 == null || s2.definitionVersion !== globals_default.currentProjectStateDefinitionVersion) throw new InvalidArgumentError("Error validating conf.state in Theatre.getProject(".concat(JSON.stringify(projectId), ", conf). The state seems to be formatted in a way that is unreadable to Theatre.js. Read more at https://www.theatrejs.com/docs/latest/manual/projects#state"));
	};
	var validateProjectIdOrThrow = (value) => {
		if (typeof value !== "string") throw new InvalidArgumentError("Argument 'projectId' in `Theatre.getProject(projectId, ...)` must be a string. Instead, it was ".concat(userReadableTypeOfValue_default(value), "."));
		const idTrimmed = value.trim();
		if (idTrimmed.length !== value.length) throw new InvalidArgumentError("Argument 'projectId' in `Theatre.getProject(\"".concat(value, "\", ...)` should not have surrounding whitespace."));
		if (idTrimmed.length < 3) throw new InvalidArgumentError("Argument 'projectId' in `Theatre.getProject(\"".concat(value, "\", ...)` should be at least 3 characters long."));
	};
	function onChange(pointer3, callback, rafDriver) {
		const ticker = rafDriver ? privateAPI(rafDriver).ticker : getCoreTicker();
		if ((0, import_dataverse18.isPointer)(pointer3)) return (0, import_dataverse19.pointerToPrism)(pointer3).onChange(ticker, callback, true);
		else if ((0, import_dataverse19.isPrism)(pointer3)) return pointer3.onChange(ticker, callback, true);
		else throw new Error("Called onChange(p) where p is neither a pointer nor a prism.");
	}
	function val7(pointer3) {
		if ((0, import_dataverse18.isPointer)(pointer3)) return (0, import_dataverse19.pointerToPrism)(pointer3).getValue();
		else throw new Error("Called val(p) where p is not a pointer.");
	}
	...
	function registerCoreBundle() {
		...
		if (typeof existingBundle !== "undefined") if (typeof existingBundle === "object" && existingBundle && typeof existingBundle.version === "string") throw new Error("It seems that the module '@theatre/core' is loaded more than once. This could have two possible causes:\n1. You might have two separate versions of Theatre.js in node_modules.\n2. Or this might be a bundling misconfiguration, in case you're using a bundler like Webpack/ESBuild/Rollup.\n\nNote that it **is okay** to import '@theatre/core' multiple times. But those imports should point to the same module.");
		else throw new Error("The variable window.".concat(coreBundle, " seems to be already set by a module other than @theatre/core."));
		const coreBundle2 = new CoreBundle();
		window[coreBundle] = coreBundle2;
		const possibleExistingStudioBundle = window[studioBundle];
		if (possibleExistingStudioBundle && possibleExistingStudioBundle !== null && possibleExistingStudioBundle.type === "Theatre_StudioBundle") possibleExistingStudioBundle.registerCoreBundle(coreBundle2);
	}
}));
